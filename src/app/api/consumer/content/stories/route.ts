...
import { getStoriesLocal } from '@/server/consumer/personalization';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stories = await getStoriesLocal(branchId);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Stories request failed' },
      { status: 500 }
    );
  }
}
