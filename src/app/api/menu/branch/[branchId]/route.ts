...
function hasMenuItems(payload: unknown) {
  const resolvedPayload = (payload as any)?.data ?? payload;
  const categories = Array.isArray((resolvedPayload as any)?.categories)
    ? (resolvedPayload as any).categories
    : [];

  return categories.some((category: any) => Array.isArray(category?.items) && category.items.length > 0);
}
...
async function loadMenuFromSupabase(branchId: string) {
  ...
  for (const modifierItem of modifierItemsResult.data || []) {
    ...
    bucket.push({
      ...
      name: String(modifierItem.name || 'Opción'),
...
export async function GET(
  ...
  try {
    ...
    const supabaseServer = getSupabaseServer();
    const { data: branchRecord } = await (supabaseServer as any)
      .from('branches')
      .select('id, restaurant_id')
    ...
    if (apiUrl && branchRecord?.restaurant_id) {
      try {
        const consumerResponse = await fetch(
          `${apiUrl.replace(/\/$/, '')}/api/consumer/v1/restaurants/${encodeURIComponent(String(branchRecord.restaurant_id))}/menu?branch_id=${encodeURIComponent(normalizedBranchId)}`,
          {
            method: 'GET',
            cache: 'no-store',
            headers: {
        ...
        const consumerPayload = await consumerResponse.json().catch(() => null);
        if (consumerResponse.ok && hasMenuItems(consumerPayload)) {
          return NextResponse.json(consumerPayload?.data || consumerPayload);
        }
      } catch {
        // Fall through to local Supabase-backed menu resolution.
      }
    }
    ...
    return NextResponse.json(await loadMenuFromSupabase(normalizedBranchId));
  } catch (error) {
